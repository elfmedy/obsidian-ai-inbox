...
当前阶段：**0.2.0 Alpha 已实现自动发现与一次配对、默认仓库切换，以及完整保存链路。** 用户已确认能看到保存的对话和图片；新版连接流程的受控端到端验证通过。长聊天、生成图和真实分支仍有验收项，尚未发布到商店或 Obsidian 社区。
...
2. 在 BRAT 设置中选择 **Add Beta plugin**，输入 `elfmedy/obsidian-ai-inbox`，添加并启用 **AI Inbox**。如选择指定版本，请使用 `0.2.0`。
3. 从 [Release](https://github.com/elfmedy/obsidian-ai-inbox/releases/tag/0.2.0) 下载 `ai-inbox-chrome-0.2.0-alpha.zip` 并解压，在 Chrome 扩展管理页打开开发者模式，加载解压后的 `ai-inbox-chrome` 文件夹。
...
开发环境使用 Node.js 24。执行 `npm ci`、`npm run check`，生成 P0 和 Alpha 两套包。检查包括官方 Obsidian lint、TypeScript strict、162 项自动测试、两端打包、版本一致性、第三方许可随包和重复构建字节比较。
